...
class Encoder(nn.Module):
    def __init__(self, nc: int = 256):
        ...
        self.maxpool = nn.Identity()  # CIFAR uses identity instead of down sampling

        # layer2_x : 3 × BasicBlock, 64→64 (no down sampling)
        # Block-1
        ...
        # Compress to nc: pool to 4x4 then 1x1 conv -> BN -> ReLU -> GAP -> L2-normalize
        self.pool4 = nn.AvgPool2d(8)  # 32 → 4
        self.to_nc = nn.Conv2d(64, nc, 1, bias=False)
        self.to_nc_bn = nn.BatchNorm2d(nc)

        # Kaiming init; zero-init residual BN2 gamma for better start
    ...
    def _basic_block(x, conv1, bn1, conv2, bn2):
        """BasicBlock without downsample; input/output channels equal."""
        identity = x
...
# Decoder: restore to 64x32x32 then run stages (layer3~5) like ResNet-34
class Decoder(nn.Module):
    r"""Map (B,nc) back to a 64×32×32 feature map, then run the last 3 stages of ResNet-34."""

    def __init__(self, nc: int = 256, num_classes: int = 10):
        super().__init__()
        # Restore from z to 64×32×32
        ...
        # layer3_x : 4 × BasicBlock, 64→128, first block stride=2
        self.ds3 = nn.Sequential(nn.Conv2d(64, 128, 1, 2, bias=False),
                                 nn.BatchNorm2d(128))
        self.l3_b1_conv1 = nn.Conv2d(64, 128, 3, 2, 1, bias=False)
        ...
        # layer4_x : 6 × BasicBlock, 128→256, first block stride=2
        self.ds4 = nn.Sequential(nn.Conv2d(128, 256, 1, 2, bias=False),
                                 nn.BatchNorm2d(256))
        self.l4_b1_conv1 = nn.Conv2d(128, 256, 3, 2, 1, bias=False)
        ...
        # layer5_x : 3 × BasicBlock, 256→512, first block stride=2
        self.ds5 = nn.Sequential(nn.Conv2d(256, 512, 1, 2, bias=False),
                                 nn.BatchNorm2d(512))
        self.l5_b1_conv1 = nn.Conv2d(256, 512, 3, 2, 1, bias=False)
        ...
        # Initialization (Kaiming) + zero-init residual BN2 gamma
        for m in self.modules():
...
def evaluate(model, loader, device):
    """Return top-1 accuracy over a dataloader; also updates model.last_* timing."""
    model.eval()
    correct = total = 0
    enc_times = []
    dec_times = []
...
def main():
    ...
    # Worker seeds for reproducibility across workers
    g = torch.Generator()
    ...
    # model_graph.visual_graph.render(filename="comm_cifar10_resnet34_match_cifar_split", format="pdf", cleanup=True)
